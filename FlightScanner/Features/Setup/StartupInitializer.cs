    ...
    ILogger<StartupInitializer> logger)
{
    ...
    private static async Task EnsureSchemaAsync(ApplicationDbContext db, CancellationToken cancellationToken)
    {
        await db.Database.ExecuteSqlRawAsync(
            ...
            ADD COLUMN IF NOT EXISTS "NameFr" character varying(120);

            ALTER TABLE "FlightLocations"
            ADD COLUMN IF NOT EXISTS "NameAr" character varying(120);

            ALTER TABLE "FlightLocations"
            ADD COLUMN IF NOT EXISTS "CountryNameFr" character varying(80);

            ALTER TABLE "FlightLocations"
            ADD COLUMN IF NOT EXISTS "CountryNameAr" character varying(80);

            ALTER TABLE "FlightLocations"
            ADD COLUMN IF NOT EXISTS "ContinentFr" character varying(32);

            ALTER TABLE "FlightLocations"
            ADD COLUMN IF NOT EXISTS "ContinentAr" character varying(32);
            ...
            SET "Code" = UPPER(TRIM("Code")),
                "Name" = TRIM("Name"),
                "NameFr" = NULLIF(TRIM(COALESCE("NameFr", '')), ''),
                "NameAr" = NULLIF(TRIM(COALESCE("NameAr", '')), ''),
                "CountryCode" = NULLIF(UPPER(TRIM(COALESCE("CountryCode", ''))), ''),
                "CountryName" = NULLIF(TRIM(COALESCE("CountryName", '')), ''),
                "CountryNameFr" = NULLIF(TRIM(COALESCE("CountryNameFr", '')), ''),
